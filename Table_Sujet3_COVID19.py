...
Adresse_table = Table(
    ...
    Column("code_postal", String(35), nullable=False),
...
Infection_table = Table(
    "Infection",
    metadata,
    Column("date_infection", Date, primary_key=True)
)
...
Dose_table = Table(
    "Dose",
    metadata,
    Column("date_administration", Date, primary_key=True),
    Column("id_vaccin", ForeignKey("Vaccin.id_vaccin"), nullable=False)
)
...
etablissement_table = Table(
    "Etablissement",
    metadata,
    Column("code_indentification", Integer, primary_key=True),
